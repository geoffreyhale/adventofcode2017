...
childrens = [];
rows.forEach((row) => {
    disc = row.substr(0,row.indexOf(' '));

    arrowPos = row.indexOf('->');
    if (arrowPos > -1) {
        children = row.substr(arrowPos+2);
        childrenArray = children.split(/[,]+/).map(child => child.trim());
        childrens.push(...childrenArray);
    }
...
myArray = discs.filter( function( el ) {
    return childrens.indexOf( el ) < 0;
});
console.log(myArray); //mwzaxaj