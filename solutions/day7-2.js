...
rows.forEach((row) => {
    ...
    const weight = parseInt(row.substr(row.indexOf('(')+1,row.indexOf(')')-1));
    const childNames = row.indexOf('->') > -1 ? row.substr(row.indexOf('->')+2).split(/[,]+/).map(child => child.trim()) : [];
    discs[name] = {
        weight: weight,
...
//build
for (discKey in discs) {
    const disc = discs[discKey];
    disc.children = {};
    disc.childNames.forEach((childName) => {
        disc.children[childName] = discs[childName];
    });
    delete disc.childNames;
};
//console.log(discs);

const haveChildren = (parents) => {
    for (key in parents) {
        if (!_.isEmpty(parents[key].children)) {
            return true;
        }
    }
    return false;
...
const weightsAllEqual = (discs) => {
    const weights = [];
    for (key in discs) {
        weights.push(discs[key].weight);
    }
    return _.uniq(weights).length <= 1;
};

const addChildrenWeightsToParentAndDeleteChildren = (disc) => {
    for (key in disc.children) {
        disc.weight += disc.children[key].weight;
        delete disc.children[key];
    }
};

const announceSolution = (disc) => {
    console.log(disc.children);
};
...
const reduce = (discs) => {
    for (discKey in discs) {
        ...
        // If disc has children but no grandchildren
        // Announce if the children's weights are all equal
        // Add the children's weights to the disc
        // Remove the children
        if (haveChildren([disc]) && !haveChildren(disc.children)) {
            if (!weightsAllEqual(disc.children)) {
                announceSolution(disc);
                break;
            }
            addChildrenWeightsToParentAndDeleteChildren(disc);
            delete discs[discKey].children;
        }
    }
    //console.log(discs);
};
...
//at announceSolution
//vrgxe has weight 2166
//its siblings have 2159
//diff is 2166 - 2159 = 7
//vrgxe should be 7 less
//vrgxe original value is 1226
//vrgxe should be 1226 - 7 = 1219
//solution: 1219 //correct