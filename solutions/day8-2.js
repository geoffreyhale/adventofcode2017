const input = `b inc 580 if bkd > -1
v inc 491 if z >= -1
...
rows.forEach((row) => {
    ...
    const regKey = rowArr[0];
    const regOp = rowArr[1];
    const regOpVal = parseInt(rowArr[2]);
    const reg2Key = rowArr[4];
    const reg2Op = rowArr[5];
    const reg2Val = parseInt(rowArr[6]);
    //console.log(row,regKey,regOp,regOpVal,reg2Key,reg2Op,reg2Val);

    if (regs[regKey] === undefined) {
        regs[regKey] = 0;
    }
    if (regs[reg2Key] === undefined) {
        regs[reg2Key] = 0;
    }

    if (eval(regs[reg2Key].toString() + reg2Op + reg2Val)) {
        if (regOp === "inc") {
            regs[regKey] += regOpVal;
        } else if (regOp === "dec") {
            regs[regKey] -= regOpVal;
        }
    }

    solution = Math.max(solution, regs[regKey]);
});

//const solution = Math.max(...Object.values(regs));
console.log(solution); //6619
